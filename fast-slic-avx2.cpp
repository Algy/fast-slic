    ...
    for (int cluster_sorted_idx = 0; cluster_sorted_idx < K; cluster_sorted_idx++) {
        ...
        for (int16_t i = 0; i < patch_height; i++) {
            ...
            for (int j = 0; j < patch_virtual_width; j += 8) {
                ...
                __m128i new_min_dist__narrow = _mm_min_epu16(old_min_dist__narrow, dist_vec__narrow);
                ...
                __m128i new_assignment__narrow = _mm_blendv_epi8(cluster_number_vec__narrow, old_assignment__narrow, mask__narrow);
                #ifdef FAST_SLIC_SIMD_INVARIANCE_CHECK
                {
                    ...
                    for (int delta = 0; delta < 8; delta++) {
                        assert(cluster_arr[delta] == cluster_number);
                    }
                    for (int delta = 0; delta < 8; delta++) {
                        if (old_dists[delta] > calcd_dists[delta]) {
                            assert(new_assignment[delta] == cluster_number);
                ...
                #endif
                if (8 < patch_virtual_width - j) {
                    _mm_storeu_si128((__m128i*)min_dist_row, new_min_dist__narrow);
                    _mm_storeu_si128((__m128i*)assignment_row, new_assignment__narrow);
                } else {
                    uint16_t new_min_dist_arr[8], new_assignment_arr[8];
                    _mm_storeu_si128((__m128i*)new_min_dist_arr, new_min_dist__narrow);
                    _mm_storeu_si128((__m128i*)new_assignment_arr, new_assignment__narrow);

                    for (int rem = 0; rem < patch_virtual_width - j; rem++) {
                        min_dist_row[rem] = new_min_dist_arr[rem];
                        assignment_row[rem] = new_assignment_arr[rem];
                    }
                }
            }
        }
